...
app.config['MYSQL_DATABASE_DB'] = 'mygiftdb'
...
def login():
    ...
    if request.method == 'POST':    # Create variables for easy access
        ...
        conn = mysql.connector.connect(host='localhost', user='root', password='root', database='mygiftdb')
        ...
        if account:
            if(account[3]=="admin@ravigiftshop.in"):
...
def Adminpage():
    ...
    cursor.execute('SELECT * FROM customers where email!="admin@vamshigiftshop.in"')
...
def add_product_to_cart():
    ...
    try:
        ...
        if _quantity and _code and request.method == 'POST':
            conn =  conn = mysql.connector.connect(host='localhost',user='root',password='root',database='mygiftdb')
            cursor = conn.cursor()
        ...
        if 'cart_item' in session:
            if request.method == 'POST':
                for key, value in session['cart_item'].items():
                    ...
                    conn = conn = mysql.connector.connect(host='localhost', user='root', password='root',
                                                          database='mygiftdb')
                    cursor = conn.cursor()
